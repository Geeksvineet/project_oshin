<?php
session_start(); // Start the session
if (!isset($_SESSION['admin_logged_in'])) {
    header('Location: ../login/login'); // Redirect to login page if not logged in
    exit();
}
?>

<?php
include('../../database/config.php');

if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "SELECT * FROM testimonials WHERE id = $id";
    $result = mysqli_query($conn, $sql);
    $category = mysqli_fetch_assoc($result);
}

if (isset($_POST['update'])) {
    $name = $_POST['name'];
    $description = $_POST['description'];
    $relation = $_POST['relation'];




    if ($_FILES['image']['name']) {
        // Path to the old image
        $oldImage = $category['image'];

        // New image upload
        $image = $_FILES['image']['name'];
        $target = "images/" . basename($image);

        // Delete the old image
        if (file_exists($oldImage)) {
            unlink($oldImage);
        }

        // Move new image to target directory
        move_uploaded_file($_FILES['image']['tmp_name'], $target);
        $updateSql = "UPDATE testimonials SET name='$name', description='$description', relation='$relation', image='$target' WHERE id=$id";
    } else {
        // If image is not updated, keep the old image
        $updateSql = "UPDATE testimonials SET name='$name', description='$description', relation='$relation' WHERE id=$id";
    }



    // Update the category in the database

    mysqli_query($conn, $updateSql);
    header('Location: show_testimonials');
}
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>edi testimonial - Oshin Sports Academy</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">


    <?php include '../../partials/header_link.php' ?>


</head>

<body>



    <div class="container-scroller">

        <?php include '../../partials/_sidebar.php' ?>

        <div class="container-fluid page-body-wrapper">
            <?php include '../../partials/_navbar.php' ?>

            <div class="main-panel">
                <div class="content-wrapper">
                    <!-- content -->
                    <div class="container mt-5">
                        <h2>Edit testimonials</h2>
                        <form method="POST" enctype="multipart/form-data">
                            <div class="form-group">
                                <label>Name</label>
                                <input type="text" class="form-control" name="name" value="<?= $category['name'] ?>" required>
                            </div>
                            <div class="form-group">
                                <label>Description</label>
                                <textarea class="form-control" name="description" required><?= $category['description'] ?></textarea>
                            </div>
                            <div class="form-group">
                                <label>Relation</label>
                                <input type="text" class="form-control" name="relation" value="<?= $category['relation'] ?>" required>
                            </div>
                            <div class="form-group">
                                <label>Current Image</label>
                                <img src="<?= htmlspecialchars($category['image']) ?>" alt="Current Image" width="100">
                            </div>
                            <div class="form-group">
                                <label>Image</label>
                                <input type="file" class="form-control" name="image">
                                <small>Leave blank if you do not want to change the image.</small>
                            </div>
                            <button type="submit" name="update" class="btn btn-primary">Update Category</button>
                            <a href="show_testimonials" class="btn btn-secondary">Cancel</a>
                        </form>
                    </div>

                </div>

                <?php include '../../partials/_footer.php' ?>

            </div>
        </div>

    </div>








    <?php include '../../partials/script_links.php' ?>
    <script src="https://code.jquery.com/jquery-3.5.1.slim.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.5.2/dist/umd/popper.min.js"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
</body>

</html>